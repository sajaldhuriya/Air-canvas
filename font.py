...
# Path to save the output video and drawing
...
while True:
    ...
    if len(lmList) != 0:
        ...
        # AI Calculation Trigger (All 5 fingers up)
        fingers_up = [lmList[i][2] < lmList[i - 2][2] for i in [8, 12, 16, 20]]
        if fingers_up == [True, True, True, True]:
            # Prepare the drawing for AI (PIL format)
